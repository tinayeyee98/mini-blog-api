...
class CardRepository:
    ...
    async def find_card(cls, card_name: str):
        try:
            card_dict: Dict[str, Any] = await cls.collection.find_one(
                dict(name=card_name)
            )
    ...
    async def update_card(
        cls, card_name: str, card_data: Dict[str, Any], author: str
    ) -> Optional[Dict]:
        try:
    ...
    async def delete_card(cls, card_name: str, author: str):
        try:
            ...
            if author_detail.username != author:
                raise HTTPException(
                    status_code=403, detail="You are unauthorized to delete this card"
                )
